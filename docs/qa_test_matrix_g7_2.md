...
| Senior companion | Validate voice gateway entry point | Senior session; voice gateway reachable | 1. Open `/senior/companion`. 2. Grant microphone permission. 3. Record and send a short question. | Audio is sent to the configured gateway and the returned answer plays. | Missing microphone permission, gateway failure not surfaced, over-claiming language. |
| Guardian insights | Validate deterministic handoff | Guardian session; active alerts/history present | 1. Open `/guardian/insights`. 2. Open alerts, summary, and timeline from the screen. | Screen clearly routes guardians to deterministic local facts without AI chat controls. | Dead links, implied guardian AI chat, invented incidents. |
...
- Senior voice companion depends on the configured voice gateway; guardian insights remain deterministic links to local facts.
