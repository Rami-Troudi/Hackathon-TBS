...
> **Note:** All API URLs are stubs in Group 0/G1/G2/G3/G4/G5. No real network calls are made.
...
## Senior feature flow (G3 + G5 wellbeing)
...
## Guardian feature flow (G4 + G5 expansion)
...
Current targeted tests (G2 + G3 + G4 + G5):
...
- **Storage policy for G2-G5.** `SharedPreferences` is reserved for
